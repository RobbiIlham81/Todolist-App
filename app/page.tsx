...
export default function Home() {
  const [interpretation, setInterpretations] = useState<IInterpretation[]>([]);
          ...
          {interpretation?.map((interpretation) => (
            <div key={interpretation.$id} className="p-4 my-2 rounded-md border-b leading-8">
              ...
              <div className="flex gap-4 mt-4 justify-end">
                <Link className="bg-slate-200 px-4 py-2 rounded-md upprcase text-sm font-bold tracking-widest" href={`/edit/${interpretation.$id}`}>
                  Edit
                </Link>

                <button onClick={() => handleDelete(interpretation.$id)} className="bg-red-500 text-white px-4 py-2 rounded-md upprcase text-sm font-bold tracking-widest">
                  Delete
                </button>
              </div>
            </div>
