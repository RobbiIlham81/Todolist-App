...
export default function EditPage({ params }: { params: { id: string } }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error(error);
      setError("Failed to delete the interpretation.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-3 flex-col">
        ...
        <button className="bg-red-500 text-white mt-2 px-4 py-1 rounded-md cursor-pointer" type="button" onClick={handleDelete} disabled={isLoading}>
